...
public class PgsqlLDataSource extends DataSqlSource<AsyncConnection> {
    ...
    protected <T> CompletableFuture<Integer> insertDB(EntityInfo<T> info, T... values) {
        ...
        return writePool.pollAsync().thenCompose((conn) -> executeUpdate(info, conn, sql, values, 0, true, attrs, objs));
    }
    ...
    protected <T> CompletableFuture<Integer> deleteDB(EntityInfo<T> info, Flipper flipper, String sql) {
        ...
        return writePool.pollAsync().thenCompose((conn) -> executeUpdate(info, conn, sql, null, fetchSize(flipper), false, null));
    }
    ...
    protected <T> CompletableFuture<Integer> clearTableDB(EntityInfo<T> info, String sql) {
        ...
        return writePool.pollAsync().thenCompose((conn) -> executeUpdate(info, conn, sql, null, 0, false, null));
    }
    ...
    protected <T> CompletableFuture<Integer> updateDB(EntityInfo<T> info, final T... values) {
        ...
        return writePool.pollAsync().thenCompose((conn) -> executeUpdate(info, conn, info.getUpdateDollarPrepareSQL(values[0]), null, 0, false, attrs, objs));
    }
    ...
    protected <T> CompletableFuture<Integer> updateDB(EntityInfo<T> info, Flipper flipper, String sql, boolean prepared, Object... params) {
        ...
        return writePool.pollAsync().thenCompose((conn) -> executeUpdate(info, conn, sql, null, fetchSize(flipper), false, null, objs));
    }
    ...
    public int directExecute(String sql) {
        return writePool.pollAsync().thenCompose((conn) -> executeUpdate(null, conn, sql, null, 0, false, null)).join();
    }
