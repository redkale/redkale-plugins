...
public class KafkaMessageProducer implements MessageProducer {
    ...
    public KafkaMessageProducer(KafkaMessageAgent messageAgent) {
        ...
        this.producer = new KafkaProducer<>(
                messageAgent.createProducerProperties(), new StringSerializer(), new ByteArraySerializer());
    ...
    public CompletableFuture<Void> sendMessage(
        ...
        if (closed.get()) {
            throw new IllegalStateException(
                    getClass().getSimpleName() + "(name=" + messageAgent.getName() + ") is closed when send " + value);
        }
        if (this.producer == null) {
            throw new IllegalStateException(getClass().getSimpleName() + "(name=" + messageAgent.getName()
                    + ") not started when send " + value);
        }
        final CompletableFuture future = new CompletableFuture();
        long s = System.currentTimeMillis();
        // if (finest) logger.log(Level.FINEST, "Kafka.producer prepare send partition=" + partition + ", msg=" +
        // message);
        String traceid = Traces.currentTraceid();
        producer.send(
                new ProducerRecord<>(topic, partition, traceid, convertMessage(convert, type, value)),
                (metadata, exp) -> {
                    ...
                    if (e > 1000 && logger.isLoggable(Level.FINE)) {
                        logger.log(
                                Level.FINE,
                                getClass().getSimpleName() + "(name=" + messageAgent.getName() + ") (mq.cost-slower = "
                                        + e + " ms)，partition=" + partition + ", msg=" + value);
                    } else if (e > 100 && logger.isLoggable(Level.FINER)) {
                        logger.log(
                                Level.FINER,
                                getClass().getSimpleName() + "(name=" + messageAgent.getName() + ") (mq.cost-slowly = "
                                        + e + " ms)，partition=" + partition + ", msg=" + value);
                    } else if (e > 10 && logger.isLoggable(Level.FINEST)) {
                        logger.log(
                                Level.FINEST,
                                getClass().getSimpleName() + "(name=" + messageAgent.getName() + ") (mq.cost-normal = "
                                        + e + " ms)，partition=" + partition + ", msg=" + value);
                    }
                    Traces.removeTraceid();
                });
