...
public class RedisCacheResult {
    ...
    public String getStringValue() {
        return bytesValue == null ? null : new String(bytesValue, StandardCharsets.UTF_8);
    }
    ...
    public <T> T getObjectValue(Type type) {
        return formatValue(bytesValue, type);
    }

    protected <T> Collection<T> getCollectionValue(boolean set, Type type) {
        ...
        for (byte[] bs : bytesList) {
            list.add(formatValue(bs, type));
        }
        return list;
    }

    protected <T> Map<String, T> getMapValue(Type type) {
        ...
        for (int i = 0; i < bytesList.size(); i += 2) {
            ...
            T val = formatValue(bs2, type);
            if (val != null) map.put(formatValue(bs1, String.class).toString(), val);
        }
        return map;
    }

    protected static <T> T formatValue(byte[] bs, Type type) {
        if (bs == null) return null;
        if (type == byte[].class) return (T) bs;
        if (type == String.class) return (T) new String(bs, StandardCharsets.UTF_8);
