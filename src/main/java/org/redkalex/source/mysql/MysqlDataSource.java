...
public class MysqlDataSource extends AbstractDataSqlSource {
	...
	protected <T> CompletableFuture<Integer> clearTableDBAsync(
			EntityInfo<T> info, final String[] tables, FilterNode node, final String... sqls) {
		if (info.isLoggable(logger, Level.FINEST)) {
			if (info.isLoggable(logger, Level.FINEST, sqls[0])) {
				if (sqls.length == 1) {
	...
	protected <T> CompletableFuture<MyResultSet> thenApplyQueryUpdateStrategy(
			...
			final Function<MyClientConnection, CompletableFuture<MyResultSet>> futureFunc) {
	...
	protected <T> CompletableFuture<MyResultSet> thenApplyInsertStrategy(
			final EntityInfo<T> info,
			final CompletableFuture<MyResultSet> future,
			final ObjectRef<MyClientRequest> reqRef,
