...
public class MysqlDataSource extends DataSqlSource {
    ...
    public MysqlDataSource(String unitName, URL persistFile, Properties readprop, Properties writeprop) {
        super(unitName, persistFile, "mysql", readprop, writeprop);
    }
    ...
    private MyClient createMyPool(String rw, Properties prop) {
        ...
        AsyncGroup ioGroup = asyncGroup != null ? asyncGroup : AsyncGroup.create("Redkalex-MyClient-IOThread-" + rw.toUpperCase(),
            workExecutor, 16 * 1024, Utility.cpus() * 4).start();
        return new MyClient(ioGroup, resourceName() + "." + rw, info.servaddr, maxConns, maxPipelines, prop, username, password, info.database, encoding, info.attributes);
    }
    ...
    protected <T> CompletableFuture<Integer> insertDB(EntityInfo<T> info, T... values) {
        ...
        if (pool.cachePreparedStatements()) {
            ...
            return thenApplyQueryUpdateStrategy(info, pool.connect(null).thenCompose(conn -> {
                ...
                CompletableFuture<MyResultSet> future = pool.writeChannel(conn, req);
                if (((MyClientConnection) conn).getStatementIndex(sql) != null) return future;
                return future.thenCompose(v -> {
                    if (v.prepare == null) return CompletableFuture.completedFuture(v);
                    v.close();
                    MyReqExtended req2 = ((MyClientConnection) conn).pollReqExtended(workThread, info);
                    req2.prepare(MyClientRequest.REQ_TYPE_EXTEND_INSERT, sql, 0, attrs, objs);
                    return pool.writeChannel(conn, req2);
                });
            })).thenApply(g -> g.getUpdateEffectCount());
        } else {
            return executeUpdate(info, info.getInsertQuestionPrepareSQL(values[0]), values, 0, true, attrs, objs);
        }
    }
    ...
    protected <T> CompletableFuture<Integer> updateEntityDB(EntityInfo<T> info, final T... values) {
        ...
        MyClient pool = writePool();
        if (pool.cachePreparedStatements()) {
            String sql = info.getUpdateQuestionPrepareSQL(values[0]);
            WorkThread workThread = WorkThread.currWorkThread();
            return thenApplyQueryUpdateStrategy(info, pool.connect(null).thenCompose(conn -> {
                MyReqExtended req = ((MyClientConnection) conn).pollReqExtended(workThread, info);
                req.prepare(MyClientRequest.REQ_TYPE_EXTEND_UPDATE, sql, 0, Utility.append(attrs, primary), objs);
                CompletableFuture<MyResultSet> future = pool.writeChannel(conn, req);
                if (((MyClientConnection) conn).getStatementIndex(sql) != null) return future;
                return future.thenCompose(v -> {
                    if (v.prepare == null) return CompletableFuture.completedFuture(v);
                    v.close();
                    MyReqExtended req2 = ((MyClientConnection) conn).pollReqExtended(workThread, info);
                    req2.prepare(MyClientRequest.REQ_TYPE_EXTEND_UPDATE, sql, 0, Utility.append(attrs, primary), objs);
                    return pool.writeChannel(conn, req2);
                });
            })).thenApply(g -> g.getUpdateEffectCount());
        } else {
            return executeUpdate(info, info.getUpdateQuestionPrepareSQL(values[0]), null, 0, false, Utility.append(attrs, primary), objs);
        }
    }
        ...
        if (info.getTableStrategy() == null && selects == null && pool.cachePreparedStatements()) {
            ...
            return thenApplyQueryUpdateStrategy(info, pool.connect(null).thenCompose(conn -> {
                ...
                return future.thenCompose(v -> {
                    ...
                    v.close();
                    MyReqExtended req2 = ((MyClientConnection) conn).pollReqExtended(workThread, info);
                    req2.prepare(MyClientRequest.REQ_TYPE_EXTEND_QUERY, sql, 0, null, new Object[]{pk});
                    return pool.writeChannel(conn, req2);
                });
        ...
        if (info.getTableStrategy() == null && selects == null && pool.cachePreparedStatements()) {
            ...
            return thenApplyQueryUpdateStrategy(info, pool.connect(null).thenCompose(conn -> {
                ...
                return future.thenCompose(v -> {
                    ...
                    v.close();
                    MyReqExtended req2 = ((MyClientConnection) conn).pollReqExtended(workThread, info);
                    req2.prepare(MyClientRequest.REQ_TYPE_EXTEND_QUERY, sql, 0, null, params);
                    req.finds = true;
                    return pool.writeChannel(conn, req2);
                });
            })).thenApply((MyResultSet dataset) -> {
        ...
        if (!needtotal) {
            ...
            if (queryallcacheflag) {
                ...
                listfuture = thenApplyQueryUpdateStrategy(info, pool.connect(null).thenCompose(conn -> {
                    ...
                    CompletableFuture<MyResultSet> future = pool.writeChannel(conn, req);
                    if (((MyClientConnection) conn).getStatementIndex(listsql) != null) return future;
                    return future.thenCompose(v -> {
                        if (v.prepare == null) return CompletableFuture.completedFuture(v);
                        v.close();
                        MyReqExtended req2 = ((MyClientConnection) conn).pollReqExtended(workThread, info);
                        req2.prepare(MyClientRequest.REQ_TYPE_EXTEND_QUERY, listsql, 0, null);
                        return pool.writeChannel(conn, req2);
                    });
                }));
            } else {
    ...
    protected <T> CompletableFuture<MyResultSet> thenApplyQueryUpdateStrategy(final EntityInfo<T> info, final CompletableFuture<MyResultSet> future) {
    ...
    protected <T> CompletableFuture<MyResultSet> thenApplyInsertStrategy(final EntityInfo<T> info, final CompletableFuture<MyResultSet> future, final String sql, final T[] values, int fetchSize, final Attribute<T, Serializable>[] attrs, final Object[]... parameters) {
        if (info == null || (info.getTableStrategy() == null && !autoddl())) return future;
        final CompletableFuture<MyResultSet> rs = new CompletableFuture<>();
        future.whenComplete((g, t) -> {
            if (t != null) while (t instanceof CompletionException) t = t.getCause();
                ...
                if (info.getTableStrategy() == null) {  //单表模式
                    ...
                    if (tablesql == null) {  //没有建表DDL
                        ...
                        writePool().sendAsync(null, createTableReq).whenComplete((g2, t2) -> {
                            ...
                            if (t2 == null) { //建表成功
                                //执行一遍新增操作
                                MyReqInsert insertReq = new MyReqInsert();
    ...
    protected <T> CompletableFuture<Integer> executeUpdate(final EntityInfo<T> info, final String sql, final T[] values, int fetchSize, final boolean insert, final Attribute<T, Serializable>[] attrs, final Object[]... parameters) {
        ...
        CompletableFuture<MyResultSet> future = pool.connect(null).thenCompose(conn -> {
            MyReqUpdate req = insert ? ((MyClientConnection) conn).pollReqInsert(workThread, info) : ((MyClientConnection) conn).pollReqUpdate(workThread, info);
