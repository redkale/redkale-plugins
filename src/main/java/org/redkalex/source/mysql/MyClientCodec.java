        ...
        while (buffer.hasRemaining()) {
            while (buffer.hasRemaining()) {
                ...
                if ((request == null || request.isVirtualType()) && !conn.isAuthenticated()) { // 首次通讯
                    // logger.log(Level.FINEST, "[" + Times.nowMillis() + "] [" + Thread.currentThread().getName() + "]:
                    // " + conn + ", length=" + length + ", index=" + index + ", remains=" + buffer.remaining());
                    try {
                        conn.handshake =
                                MyRespHandshakeDecoder.instance.read(conn, buffer, length, index, array, request, null);
                ...
                if (MysqlDataSource.debug && conn.isAuthenticated()) {
                    logger.log(
                            Level.FINEST,
                            "[" + Times.nowMillis() + "] ["
                                    + Thread.currentThread().getName() + "]: " + conn + ", length=" + length
                                    + ", index=" + index + ", typeid=0x" + Integer.toHexString(typeid) + ", status="
                                    + (lastResult == null ? -1 : lastResult.status) + ", position=" + buffer.position()
                                    + ", remains=" + buffer.remaining());
                }
                    ...
                    if (reqType == REQ_TYPE_AUTH) { // 登录
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                ...
                                                + ", buffernot enough, continue read, remain=" + buffer.remaining());
                            }
                        ...
                        if (typeid == 0xFE) { // AUTH_SWITCH_REQUEST_STATUS_FLAG
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", length=" + length
                                                + ", index=" + index + ", typeid=0x" + Integer.toHexString(typeid)
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", length=" + length
                                                + ", index=" + index + ", typeid=0x" + Integer.toHexString(typeid)
                                                + ", pluginName=" + pluginName + ", position=" + buffer.position()
                                                + ", remains=" + buffer.remaining());
                            }
                            authResult.authSwitch = new MyReqAuthentication.MyReqAuthSwitch(
                                    pluginName, nonce, ((MyReqAuthentication) request).password);
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn
                                                + ", auth OK! remain = " + buffer.remaining());
                            }
                        } else {
                            lastExc = new SQLException("Not support auth method, typeid: " + typeid);
                        }
                        ...
                        if (MysqlDataSource.debug && conn.isAuthenticated()) {
                            logger.log(
                                    Level.FINEST,
                                    "[" + Times.nowMillis() + "] ["
                                            + Thread.currentThread().getName() + "]: " + conn + ", authResult="
                                            + authResult);
                        }
                    } else if (reqType == REQ_TYPE_UPDATE
                            || reqType == REQ_TYPE_INSERT
                            || reqType == REQ_TYPE_DELETE
                            || reqType == REQ_TYPE_BATCH) {
                        // 简单更新 开始
                        ...
                        if (MysqlDataSource.debug && buffer.position() != bufpos + length) {
                            logger.log(
                                    Level.FINER,
                                    "[" + Times.nowMillis() + "] ["
                                            + Thread.currentThread().getName() + "]: " + conn + ", request=" + request
                                            + ", buffer-currpos: " + buffer.position() + ", startpos=" + bufpos
                                            + ", length=" + length);
                        }
                        buffer.position(bufpos + length);
                        if (MysqlDataSource.debug && conn.isAuthenticated()) {
                            logger.log(
                                    Level.FINEST,
                                    "[" + Times.nowMillis() + "] ["
                                            + Thread.currentThread().getName() + "]: " + conn + ", affectedRows="
                                            + ok.affectedRows + ", statusFlags=" + ok.serverStatusFlags
                        ...
                        if (lastResult.status == STATUS_QUERY_ROWDESC) {
                            lastResult.rowDesc.columns[lastResult.rowColumnDecodeIndex] =
                                    MyRespRowColumnDecoder.instance.read(
                                            conn, buffer, length, index, array, request, lastResult);
                            buffer.position(bufpos + length);
                            // logger.log(Level.FINEST, "[" + Times.nowMillis() + "] [" +
                            // Thread.currentThread().getName() + "]: " + conn + ", column = " +
                            // lastResult.rowDesc.columns[lastResult.rowColumnDecodeIndex]);
                            ...
                            MyRowData row = MyRespRowDataRowDecoder.instance.read(
                                    conn, buffer, length, index, array, request, lastResult);
                            // row = null 表示数据读取完毕
                            if (row != null) {
                            ...
                            if (buffer.position() != bufpos + length) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn
                                                + ", rowdata buffer-currpos : " + buffer.position()
                                                + ", unread-byte-len=" + (bufpos + length - buffer.position())
                                                + ", startpos=" + bufpos + ", length=" + length);
                            }
                            buffer.position(bufpos + length);
                            if (row != null) {
                                continue;
                        ...
                        if (lastResult.status == STATUS_PREPARE_PARAM) {
                            ...
                            MyRowColumn column = MyRespRowColumnDecoder.instance.read(
                                    conn, buffer, length, index, array, request, lastResult);
                            buffer.position(bufpos + length);
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", paramColumn = "
                                                + column);
                            }
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", sendBindCount = "
                                                + reqext.getBindCount() + ", 解析完paramColumn后缓存MyPrepareDesc = "
                                                + prepare);
                            }
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", columnColumn = "
                                                + column);
                            }
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn
                                                + ", 解析完columnColumn后缓存MyPrepareDesc = " + prepare + "\r\n");
                            }
                            if (reqext.getBindCount() > 0) {
                                continue;
                            }
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", extendColumn = "
                                                + column);
                            }
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", 读取到一条RowData: "
                                                + row);
                            }
                            if (row != null) {
                                lastResult.addRowData(row);
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", rowData 读取完毕. ");
                            }
                            lastResult.status = 0; // row全部读取完毕
                            lastResult.effectRespCount++;
                            ...
                            if (length == 7) {
                                ...
                                if (buffer.position() != bufpos + length) {
                                    logger.log(
                                            ...
                                            "[" + Times.nowMillis() + "] ["
                                                    + Thread.currentThread().getName() + "]: " + conn
                                                    + ", prepareok buffer-currpos: " + buffer.position()
                                                    + ", unread-byte-len=" + (bufpos + length - buffer.position())
                                                    + ", startpos=" + bufpos + ", length=" + length);
                                }
                                buffer.position(bufpos + length);
                            ...
                            MyRespPrepare prepare = MyRespPrepareDecoder.instance.read(
                                    conn, buffer, length, index, array, request, lastResult);
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        Level.FINEST,
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn + ", request.sql ="
                                                + reqext.sql + ", prepare = " + prepare);
                            }
                            ...
                            if (buffer.position() != bufpos + length) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn
                                                + ", sendPrepare buffer-currpos : " + buffer.position()
                                                + ", unread-byte-len=" + (bufpos + length - buffer.position())
                                                + ", startpos=" + bufpos + ", length=" + length);
                            }
                            buffer.position(bufpos + length);
                            ...
                            if (MysqlDataSource.debug) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn
                                                + ", RowDesc预编译查询column数量: " + columnCount);
                            }
                            lastResult.rowDesc = new MyRowDesc(new MyRowColumn[columnCount]);
                            lastResult.rowColumnDecodeIndex = 0;
                            ...
                            if (MysqlDataSource.debug && conn.isAuthenticated()) {
                                logger.log(
                                        ...
                                        "[" + Times.nowMillis() + "] ["
                                                + Thread.currentThread().getName() + "]: " + conn
                                                + ", 增删改结果: affectedRows=" + ok.affectedRows + ", statusFlags="
                                                + ok.serverStatusFlags + ", warningCount=" + ok.warningCount);
                            }
                            if (lastResult.effectRespCount < reqext.getBindCount()) {
                                continue;
                            }
                ...
                if (buffer.position() != bufpos + length) {
                    logger.log(
                            Level.SEVERE,
                            "[" + Times.nowMillis() + "] ["
                                    + Thread.currentThread().getName() + "]: " + conn + ", buffer-currpos : "
                                    + buffer.position() + ", unread-byte-len=" + (bufpos + length - buffer.position())
                                    + ", startpos=" + bufpos + ", length=" + length);
                }
                halfFrameBytes = null;
                halfFrameLength = 0;
                if (lastExc != null) {
                    ...
                    if (MysqlDataSource.debug) {
                        logger.log(
                                Level.FINEST,
                                "[" + Times.nowMillis() + "] ["
                                        + Thread.currentThread().getName() + "]: " + conn + ", 返回结果异常 = " + lastExc);
                    }
                } else if (lastResult != null) {
                    addMessage(request, lastResult);
                    if (MysqlDataSource.debug) {
                        logger.log(
                                Level.FINEST,
                                "[" + Times.nowMillis() + "] ["
                                        + Thread.currentThread().getName() + "]: " + conn + ", 返回结果 = " + lastResult
                                        + ", request = " + (request == null ? null : request.toSimpleString()));
                    }
                    lastResult = null;
                } else {
                    if (MysqlDataSource.debug) {
                        logger.log(
                                Level.FINEST,
                                "[" + Times.nowMillis() + "] ["
                                        + Thread.currentThread().getName() + "]: " + conn + ", 返回无结果");
                    }
                }
                lastExc = null;
                request = null;
