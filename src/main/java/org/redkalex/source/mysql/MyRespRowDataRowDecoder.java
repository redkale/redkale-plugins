    ...
    private Serializable getColumnData(MyRowColumn column, ByteBuffer buffer) {
        switch (column.type) {
            case FIELD_TYPE_TINY:
                return column.unsign ? buffer.get() & 0xff : buffer.get();
            case FIELD_TYPE_SHORT:
                return column.unsign ? Mysqls.readUB2(buffer) : Mysqls.readShort(buffer);
            case FIELD_TYPE_LONG:
                return column.unsign ? Mysqls.readUB4(buffer) : Mysqls.readInt(buffer);
            case FIELD_TYPE_LONGLONG:
                return column.unsign ? Mysqls.readLong(buffer) : Mysqls.readLong(buffer);
            case FIELD_TYPE_FLOAT:
                return Float.intBitsToFloat(Mysqls.readInt(buffer));
            case FIELD_TYPE_DOUBLE:
                return Double.longBitsToDouble(Mysqls.readLong(buffer));
