...
public class VertxSqlDataSource extends AbstractDataSqlSource {
    ...
    protected <T> CompletableFuture<Integer> insertDBAsync(EntityInfo<T> info, T... values) {
        ...
        final ObjectReference<Handler<AsyncResult<RowSet<Row>>>> selfHandlerRef = new ObjectReference<>();
        final Handler<AsyncResult<RowSet<Row>>> handler = (AsyncResult<RowSet<Row>> event) -> {
            ...
            if (event.failed()) {
                ...
                if (info.getTableStrategy() == null) { //单库单表
                    ...
                    final ObjectReference<Handler<AsyncResult<RowSet<Row>>>> createHandlerRef = new ObjectReference<>();
