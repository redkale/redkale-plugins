...
public class PgReqAuthScramSaslFinal extends PgClientRequest {
    ...
    public PgReqAuthScramSaslFinal(PgReqAuthScramSaslContinueResult bean) {
        ...
        byte[] saltedPassword =
                saltedPassword(bean.password, Base64.getDecoder().decode(bean.salt), bean.iteration);
        ...
        String cbind =
                "c=" + Base64.getEncoder().encodeToString("n,,".getBytes(StandardCharsets.UTF_8)) + ",r=" + fullNonce;
        ...
        if (msgs[0].startsWith("v=")) {
            ...
            if (!Arrays.equals(localsign, verifier)) {
                throw new IllegalArgumentException(
                        "Invalid server SCRAM signature in " + msgs[0] + ", buffer.remain = " + buffer.remaining());
            }
