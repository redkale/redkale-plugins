...
public final class FacebookPayService extends AbstractPayService {
    ...
    public CompletableFuture<PayNotifyResponse> notifyAsync(PayNotifyRequest request) {
        if (request.getAttach() != null && request.getAttach().containsKey("bean")) {
            ...
            byte[] sig =
                    Base64.getDecoder().decode(signatures[0].replace('-', '+').replace('_', '/'));
            Map<String, String> map = JsonConvert.root()
                    .convertFrom(
                            JsonConvert.TYPE_MAP_STRING_STRING,
                            Base64.getDecoder().decode(signatures[1]));
    ...
    protected boolean checkSign(
            PayElement element, Map<String, ?> map, String text, Map<String, Serializable> respHeaders) {
        String[] signatures = text.split("\\.");
        byte[] sig = Base64.getDecoder().decode(signatures[0].replace('-', '+').replace('_', '/'));
        Map<String, String> smp = JsonConvert.root()
                .convertFrom(
                        JsonConvert.TYPE_MAP_STRING_STRING, Base64.getDecoder().decode(signatures[1]));
    ...
    public static class FacebookElement extends PayElement {
        ...
        public static Map<String, FacebookElement> create(Logger logger, Properties properties) {
            ...
            String def_secret =
                    properties.getProperty("pay.facebook.secret", "").trim();
            String def_notifyurl =
                    properties.getProperty("pay.facebook.notifyurl", "").trim();

            final Map<String, FacebookElement> map = new HashMap<>();
            properties.keySet().stream()
                    .filter(x -> x.toString().startsWith("pay.facebook.")
