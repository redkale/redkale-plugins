...
public class RedisCacheSourceTest extends RedisAbstractTest {
    ...
    public void run() throws Exception {
        ...
        RedisCacheClient client = new RedisCacheClient(
                ...
                null,
                null);
        RedisCacheConnection conn =
                (RedisCacheConnection) client.createClientConnection(asyncGroup.newTCPClientConnection());
        ...
        {
            respResults.clear();
            byte[] data = "*2  $1  9  *4  $4  key1  $2  10  $4  key2  $2  30 "
                    .replaceAll("\\s+", "\r\n")
                    .getBytes();
        ...
        }
        {
            ...
            codec.decodeMessages(
                    ByteBuffer.wrap("*2  $1".replaceAll("\\s+", "\r\n").getBytes()), array);
            codec.decodeMessages(
                    ByteBuffer.wrap("  9  *4  $4  key1  $2  10  $4  key2  $2  30 "
                            .replaceAll("\\s+", "\r\n")
                            .getBytes()),
                    array);
            System.out.println("had result 1: " + respResults.size());
        ...
        }
        {
            respResults.clear();
            codec.decodeMessages(
                    ByteBuffer.wrap("*2  $".replaceAll("\\s+", "\r\n").getBytes()), array);
            codec.decodeMessages(
                    ByteBuffer.wrap("1  9  *4  $4  key1  $2  10  $4  key2  $2  30 "
                            .replaceAll("\\s+", "\r\n")
                            .getBytes()),
                    array);
            System.out.println("had result 1: " + respResults.size());
